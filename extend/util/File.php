...
class File{
	...
	* 
	*/
	static public function get_list($dir){
		$data=self::get_dirs($dir);
		$files=[];
		//系统已知图标列表
		$icon=['folder','html','jpg','png','gif'];
		//文件夹处理
		foreach ($data['dir'] as $v) {
			if($v!='.'&&$v!='..'){
				$files[]=[
					'name'=>$v,
					'type'=>'folder',
					'size'=>self::get_size($dir.$v),
					'time'=>filectime($dir.$v),
					'icon'=>'folder'
				];
			}
		}
		...
		foreach ($data['file'] as $k=>$v) {
			$extension=strtolower(pathinfo($dir.$v)['extension']);
			$files[]=[
				'name'=>$v,
				'type'=>$extension,
				'size'=>filesize($dir.$v),
				'time'=>filectime($dir.$v),
				'icon'=>in_array($extension,$icon)?$extension:'unknow'
			];
		}
		return $files;		
	}
